		...
		foreach($catNums as $cn)
		{
		  ...
		  {	//	Show current extended fields
			foreach($extendedList[$cn] as $ext)
			{
			  ...
			  {
				$text .= "
						<input type='image' alt='' title='".EXTLAN_26."' src='".e_IMAGE."/admin/up.png' name='up' value='{$ext['user_extended_struct_id']}.{$ext['user_extended_struct_order']}.{$ext['user_extended_struct_parent']}' />
						";
			  }
			  if($i <= count($extendedList[$cn])-2)
			  {
				$text .= "<input type='image' alt='' title='".EXTLAN_25."' src='".e_IMAGE."/admin/down.png' name='down' value='{$ext['user_extended_struct_id']}.{$ext['user_extended_struct_order']}.{$ext['user_extended_struct_parent']}' />";
			  }
			  $text .= "
					</form>
					</td>
	...
	function show_categories($current)
	{
		...
		if(count($catList))
		{
			...
			foreach($catList as $ext)
			{
				...
				{
					$text .= "
					<input type='image' alt='' title='".EXTLAN_26."' src='".e_IMAGE."/admin/up.png' name='catup' value='{$ext['user_extended_struct_id']}.{$i}' />
					";
				}
				if($i <= count($catList)-2)
				{
					$text .= "<input type='image' alt='' title='".EXTLAN_25."' src='".e_IMAGE."/admin/down.png' name='catdown' value='{$ext['user_extended_struct_id']}.{$i}' />";
				}
				$text .= "
				</form>
				</td>
