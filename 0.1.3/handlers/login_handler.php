...
class userlogin {
	function userlogin($username, $userpass, $autologin) 
	{
		...
		*/
		...
		{
			define("LOGINMESSAGE", LAN_LOGIN_MASSAGE2."<br /><br />");
			return FALSE;
		}
		...
		if ($pref['logcode'] && extension_loaded("gd")) 
		{
			...
			{
				define("LOGINMESSAGE", LAN_LOGIN_MASSAGE4."<br /><br />");
				return FALSE;
			}
		}
		...
		{
			define("LOGINMESSAGE", LAN_LOGIN_MASSAGE1."<br /><br />");
			$sql->db_Insert('generic', "0, 'failed_login', '".time()."', 0, '{$fip}', 0, '".$tp->toDB(LAN_LOGIN_MASSAGE6.' ::: '.LAN_LOGIN_USERNAME.': '.$username)."' ");
			$this->checkibr($fip);
			return FALSE;
		}
			...
			define("LOGINMESSAGE", LAN_LOGIN_MASSAGE3."<br /><br />");
               	$sql->db_Insert('generic', "0, 'failed_login', '".time()."', 0, '{$fip}', 0, '".$tp->toDB(LAN_LOGIN_MASSAGE7.' ::: '.LAN_LOGIN_USERNAME.': '.$username)."' ");
				$this -> checkibr($fip);
			return FALSE;
		} else {
			...
			if ($ret!='') {
				...
				if($pref['disallowMultiLogin']) {
					if($sql -> db_Select("online", "online_ip", "online_user_id='".$user_id.".".$user_name."'")) {
						define("LOGINMESSAGE", LAN_LOGIN_MASSAGE1."<br /><br />");
						$sql->db_Insert('generic', "0, 'failed_login', '".time()."', 0, '{$fip}', '$user_id', '".$tp->toDB(LAN_LOGIN_MASSAGE8.' ::: '.LAN_LOGIN_USERNAME.': '.$username.', '.LAN_LOGIN_MASSAGE9.': ').md5($ouserpass)."' ");
						$this -> checkibr($fip);
						return FALSE;
					}
				}
	...
	function checkibr($fip) 
	{
		...
		{ // Flood + Login or Login Only.
			...
			if($fails > 10) 
			{
				$sql -> db_Insert("banlist", "'$fip', '1', '".LAN_LOGIN_MASSAGE10."' ");
		   		$sql->db_Insert('generic', "0, 'auto_banned', '".time()."', 0, '{$fip}', 0, '".$tp->toDB(LAN_LOGIN_MASSAGE11.': '.$username.', '.LAN_LOGIN_MASSAGE9.': ').md5($ouserpass)."' ");
			}
		}
	}
