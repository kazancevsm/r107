...
if (!USER)
{
	...
	if ($use_imagecode)
	{
		$LOGIN_TABLE_SECIMG_LAN = LAN_LOGIN_IMAGECODE;
	...
	$LOGIN_TABLE_AUTOLOGIN_LAN = LAN_LOGIN_REMEMBER;
	$LOGIN_TABLE_SUBMIT = "<input class='button' type='submit' name='userlogin' value=\"".LAN_LOGIN_LOGIN."\" /><input type='hidden' name='e-token' value='".e_TOKEN."' />";

	$login_message = LAN_LOGIN_PROTECTED." | ".SITENAME;
...
//	echo preg_replace("/\{(.*?)\}/e", 'varset($\1,"\1")', $LOGIN_TABLE_HEADER);
	...
	if ($pref['user_reg'])
	{
		$LOGIN_TABLE_FOOTER_USERREG = "<a href='".e_SIGNUP."'>".LAN_LOGIN_USERNEW."</a>";
	}
