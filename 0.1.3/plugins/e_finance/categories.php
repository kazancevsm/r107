...
if (IsSet($cat) && $cat == 0) $total_items = $vtsql1 -> db_Select("vt_nom", "*", "nom_vis='".VT_YES."'");
...
//expression ? true_value : false_value
	$sort = $_GET['sort'];
	if ($sort==1) $sorting = 'ORDER BY `nom_price1` ASC';
	if ($sort==2) $sorting = 'ORDER BY `nom_price1` DESC';
...
//	$text .= "<font size=1 color=#00d400><h1>$cat_name</h1></font>";
	...
	if ($cat_img == ''){
	$text .= "<font size=4 color=#aaa><b>$cat_name</b></font>";
	}
	if (!$cat_desc == '') {
...
//	$text .= "<div id='vt_cat_desc_block'>$parseBB<br><a href=# onclick=\"document.getElementById('vt_cat_desc_block').style.display='none'; return false;\"><img src='theme/but_desc_none.png' align=right></a></div>";
	...
	$vtsql2 = new db;
	$vtsql2 -> db_Select("vt_nom", "*", "nom_id in (SELECT `index_nomid` FROM ".MPREFIX."vt_index WHERE index_catid='$cat') AND nom_vis='".VT_YES."' $sorting LIMIT $from,$conf_showrowsitems");
}

// ======output all nomenklature================================================
if (IsSet($cat) && $cat == 0) {
	$vtsql2 = new db;
	$vtsql2 -> db_Select("vt_nom", "*", "WHERE nom_vis='".VT_YES."' $sorting LIMIT $from,$conf_showrowsitems", "no-where");
	$caption_section = "";
}
	...
	$text .= "<td width=250px valign=top><a href='".e_PLUGIN."vtrade/vtrade.php?page=det&id=$nom_id'><font size=2><b> $nom_name</b></font></a></br>";
			
	$text .= "$nom_desc_mini <br><a href='".e_PLUGIN."vtrade/vtrade.php?page=det&id=$nom_id'>Подробное описание...</a></td>";
	...
	if ($nom_amount == ""){
		...
		$text .="<input type='hidden' name='' value='$basket_price'>";
 		$text .="<br><br><input id='tbox' type='text' size='2' name='nom_amount' value='$nom_amount'>";
 		$text .="<input type='submit' size='2' value='".VT_BUT_ADDBASK."' name='add'>";
