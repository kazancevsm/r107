...
$ok = (e_QUERY == '' || '../'.substr(e_QUERY, 0, strlen(e_FILE)-3) == e_FILE);
if($path == "/" || !$ok)
{
	$path = $adchoice[0];
...
while ($files[$c]) {
	...
	if ($pubfolder || e_QUERY == ""){
		...
		if(e_QUERY != str_replace("../","",e_FILE)){
        	$movechoice[] = e_FILE;
		}
		if(e_QUERY != str_replace("../","",e_FILE."banners/")){
        	$movechoice[] = e_FILE."banners/";
		}
	...
	$root_dir = "<a href=".e_SELF.">root</a>";
$ns->tablerender(FMLAN_29.": <b>$root_dir/".$pathd."</b>&nbsp;&nbsp;[ ".count($dirs)." ".$dstr.", ".count($files)." ".$cstr." ]", $text);
