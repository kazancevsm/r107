...
//==================== the project started in March 2012 =========================
	...
	$caption_section = "- <a href='".e_PLUGIN."vtrade/vtrade.php?page=order'>Текущий заказ</a>";
...
if (USER==TRUE){
	...
	$discount_change .="<input type='submit' size='2' value='".VT_BUT_AGR."' name='submit_discount_change'>";
...
if (USER==FALSE){
	...
	$discount_change = VT_MES_DISCOUNT;
}
...
$text .="<form enctype='multipart/form-data' name='form_order' id='div_form' style='display:block' method='post' action='".$PHP_SELF."'>";
	...
	if ($_SESSION['basket'] !='0'){
		...
		$text .="<td>$num</td>";
		$text .="<td><input type='hidden' name='id[]' value='$nom_id'><a href='".e_PLUGIN."vtrade/vtrade.php?page=det&id=$nom_id'>$nom_name</a></td>";
...
//	$text .="<script type='text/javascript'>alert ('Ваш заказ № $order_id от ".strftime('%d,%m,%y',$order_date)." отправлен администратору сайта. В ближайшее время с вами свяжутся наши менеджеры.');</script>";
	...
	header ("Location: ".e_PLUGIN."vtrade/vtrade.php?page=notify&mes_sending&$order_id&$order_date");
	exit;
	}
}
?> 