...
if(IsSet($_POST['cat_delete'])) {
	...
	$folder = $pref['mygallery_folder'];
	$gallery = urldecode("".$folder."/".$mygall_qs[1]."/".$mygall_qs[2]."");

	if  (deltree($gallery)) {
		$db_del_msg = $mydb->db_Delete("my_gallery", "img_name = '".$gallery."'");
...
if(IsSet($_POST['deletecat'])) {
    ...
    foreach(array_keys($_POST['checked']) as $gallery) {
		...
		if  (deltree($gallery)) {
			$db_del_msg = $mydb->db_Delete("my_gallery", "img_name like '".$gallery."%'");
...
if (isset($_POST['add'])) {
  ...
  if ($_POST['file_name'] && $_POST['file_description'] && $_POST['gallery_sections']) {
	...
	if (isset($_FILES['file_userfile']['error']))
	{
		...
		if ($uploaded = file_upload(e_PLUGIN."my_gallery/".$image_url, "unique"))
		{
			  ...
			  if ($upload['error'] == 0)
			  {
				if(strstr($upload['type'], "image"))
				{
                  ...
                  if ($pref['mg_view_create']) {
	                ...
	                if ($sql->db_Insert("my_gallery", $sql_text)) $message .= ", public in DB";
                    $message .= "<br />";
				}
			  }
			  else
...
if ($array = gd_info ()) {
  ...
  } else { $gd_message .=  ", JPG Support - Disabled"; }
} else { $gd_message =  "GD not support!!!"; }
...
$text = "
	...
	<form enctype='multipart/form-data' method='post' action='".e_SELF."?add'>
	<table style='".USER_WIDTH."' class='fborder'>
	<tr>
	<td class='forumheader4'>".MYGAL_L046."</td>
...
foreach ($nav_a as $folder_a)
        {
        $text .= "<optgroup label='".($folder_name[$folder."/".$folder_a] != "" ? "".$folder_name[$folder."/".$folder_a]."" : "".$folder_a."")."'>";
        $nav_b = "";
        if ($handle = opendir("$folder/$folder_a"))
            {
            while (false !== ($folder_b = readdir($handle)))
        ...
        foreach ($nav_b as $folder_b)
                {
                $text .= "<option value='".$folder."/".$folder_a."/".$folder_b."/'>".($folder_name[$folder."/".$folder_a."/".$folder_b] != "" ? "".$folder_name[$folder."/".$folder_a."/".$folder_b]."" : "".$folder_b."")."</option>";
                }
        $text_nav .= "</optgroup>";
        }
$text .= "</select></td></tr>";
	...
	$text .= "<tr>
	         	<td class='forumheader3'>".MYGAL_L036.":</td>
	         	<td class='forumheader3'><input class='tbox' name='file_name' type='text' size='50' maxlength='100' /></td>
	         </tr>

	         <tr>
	         	<td class='forumheader3'>".MYGAL_L037.":</td>
	         	<td style='width:70%' class='forumheader3'><textarea class='tbox' style='width:90%' name='file_description' cols='59' rows='6'></textarea></td>
	         </tr>

	         <tr>
	         	<td class='forumheader3'>".MYGAL_L027."</td>
	         	<td class='forumheader3'>
					<div id='fiupsection'>
						<span id='fiupopt'>
	         ...
	         <tr>
			    ...
			    <tr>
			      <td class='forumheader3' colspan='2' style='text-align:center;'>
			        <input class='button' type='submit' name='add' value='".MYGAL_L066."' />
			      </td>
			    </tr>
...
$button_text = "
    <tr>
      <td class='forumheader4' style='text-align:center;'>
        ...
        <select name='move_gallery' class='tbox' >".$selector_text2."</select>
...
$text .= "<form enctype='multipart/form-data' method='post'
          ...
          : urldecode($mygall_qs[2]))
          ."' />
          ".(($l == 0 and $mygall_qs[2] != "") ? "".MYGAL_L078." <input class='button' type='submit' name='cat_delete' value='".MYGAL_L065."' />" : "")."
          </td></tr>";
...
$text = "
	<div style='text-align:center'>
	<form enctype='multipart/form-data' method='post' action='".e_SELF."?delcat'>
	<table style='".USER_WIDTH."' class='fborder'>
		<tr>
			<td class='forumheader4'  colspan='2'>".MYGAL_L080."</td>
		</tr>";
...
foreach ($nav_a as $folder_a)
        ...
        $text .= "
        <tr>
        	<td  class='forumheader4' style='width:5%'>
        		<input type='checkbox' name='checked[".$folder."/".$folder_a."]' value='1' />
        	</td>
        	<td class='forumheader4'>
        		<b>".($folder_name[$folder."/".$folder_a] != "" ? $folder_name[$folder."/".$folder_a] : $folder_a."")."</b>
        	</td>
        </tr>";
        ...
        foreach ($nav_b as $folder_b)
                ...
                <tr>
                	<td class='forumheader3'>
                		<input type='checkbox' name='checked[".$folder."/".$folder_a."/".$folder_b."]' value='1' />
                	</td>
                	<td class='forumheader3'>
                		".($folder_name[$folder."/".$folder_a."/".$folder_b] != "" ? $folder_name[$folder."/".$folder_a."/".$folder_b] : $folder_b)."
                	</td>
...
$text .= "
		<tr>
			<td class='forumheader3' colspan='2'>
				<input class='button' type='submit' name='deletecat' onclick=\"return jsconfirm('".MYGAL_L081."')\" value='".MYGAL_L065."' />
			</td>
	</table>
	</form>
	</div>
...
//========== Create Catalog/Folder form ============
if ($action == 'createfolder') {
$folder_name = "Gallery_".date('Y-m-d')."";
$gallery_name = "".MYGAL_L003."_".date('Y-m-d')."";

$text = "<div style='text-align:center'>
	<form enctype='multipart/form-data' method='post' action='".e_SELF."?manage'>
	<table style='".USER_WIDTH."' class='fborder'>

    <tr>
      <td class='forumheader4' colspan='2' style='text-align:left;'>
         ".MYGAL_L076." -> ".$mygall_qs[1]."
      </td>
    </tr>

    <tr>
    <td class='forumheader3'>".MYGAL_L046."</td>
    <td class='forumheader3'><input class='tbox' type='text' name='folder_name' size='50' value='".$folder_name."'></td>
    </tr>

    <tr>
    <td class='forumheader3'>".MYGAL_L036.":</td>
    <td class='forumheader3'><input class='tbox' type='text' name='gallery_name' size='50' value='".$gallery_name."'></td>
    </tr>

    <tr>
      <td class='forumheader4' colspan='2' style='text-align:center;'>
        <input type='hidden' name='top_folder' value='".$mygall_qs[1]."' />
        <input class='button' type='submit' name='createfolder' value='".MYGAL_L035."' />
      </td>
    </tr>
...
//  }
  ...
  $var['img_upload']['link'] = "".e_SELF."?add";
  ...
  $var['delcat']['text'] = MYGAL_L079;
  $var['delcat']['link'] ="".e_SELF."?delcat";
