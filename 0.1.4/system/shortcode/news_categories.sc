...
$nbr_cols = (defined("NEWSCAT_COLS")) ? NEWSCAT_COLS : $nbr_cols;
	...
	while ($row3 = $sql2->db_Fetch()) {
		...
		$replace[0] = ($category_icon) ? "<a href='".e_HTTP."content.php?cat.".$category_id."'><img src='".e_IMAGE_ABS."icons/".$category_icon."' alt='' style='".$param['caticon']."' /></a>" : "";

		$search[1] = "/\{NEWSCATEGORY\}(.*?)/si";
		$replace[1] = ($category_name) ? "<a href='".e_HTTP."content.php?cat.".$category_id."' style='".$param['catlink']."' >".$tp->toHTML($category_name,TRUE,"defs")."</a>" : "";
