...
//		$menu_installed = $sql -> db_Select("menus", "menu_id", "(menu_name = 'list_new_menu' || menu_name = 'list_recent_menu') AND menu_location != '0' AND menu_class REGEXP '".e_CLASS_REGEXP."' ");
	...
	function getBullet($sectionicon, $mode)
	{
		...
		{
			if(file_exists(THEME."icons/bullet2.gif"))
			{
				$default_bullet = "<img src='".THEME_ABS."icons/bullet2.gif' alt='' />";
			}
		}
		...
		if($list_pref[$mode."_icon_use"])
		{
			if($sectionicon)
			{
				if(file_exists($listplugindir."icons/".$sectionicon))
				{
					$bullet = "<img src='".$listplugindir."icons/".$sectionicon."' style='width:".$icon_width."px; height:".$icon_height."px; border:0; vertical-align:middle;' alt='' />";
				}
			}
		}
